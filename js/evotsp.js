(function evoTSPwrapper($) {
  const baseUrl =
    "https://y7n4d21u35.execute-api.us-east-1.amazonaws.com/prod";
  ...
  function displayRoute(result) {
	...
	$('#new-route-list').append(`<li>We found the best route ${routeId} with length ${len}.</li>`);
  }
  ...
  function displayBestRoutes(bestRoutes, dbp_cb) {
    // FILL THIS IN
	console.log('New route received from API: ', bestRoutes);
	$('#best-route-list').text('')
	bestRoutes.forEach(element => {
		const {routeId, len} = element;
		$('#best-route-list').append(`<li>We found the best route ${routeId} with length ${len}.</li>`);
	})
	dbp_cb(null, bestRoutes);
  }
